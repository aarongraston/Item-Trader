	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				half4 col = tex2D(Input in, _MainTex, i.uv) * _Tint;// texture times tint;
				half depth = LinearEyeDepth(SAMPLE_DEPTH_TEXTURE_PROJ(_CameraDepthTexture, UNITY_PROJ_COORD(i.scrPos))); // depth
				half4 foamLine = 1 - saturate(_Foam * ((IN.worldPos.z - 1000) / 100 - i.worldPos.w));// foam line by comparing depth and screenposition
